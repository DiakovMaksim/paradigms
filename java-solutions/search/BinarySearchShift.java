...
public class BinarySearchShift {
    ...
    // (foreach i in 2..k) {Integer.parseInt(args[i]) < Integer.parseInt(args[i-1])} &&
    // (foreach i in k+2..args.length-1) {Integer.parseInt(args[i]) < Integer.parseInt(args[i-1])}
    // Post: R == k
    public static void main(String[] args) {
        ...
        if (a.length < 2 || a[0] > a[a.length - 1]) {
            ...
            // (foreach i in 1..args.length-1) {a[i-1] = Integer.parseInt(args[i]) &&
            // (foreach i in 1..k-1) {a[i] < a[i-1]} &&
            // (foreach i in k+1..a.length-1) {a[i] < a[i-1]} && args.length > 1 && a.length > 0
            if (s % 2 == 0) {
                // args.length > 0 && (foreach s : args) {s - > int} && x = Integer.parseInt(args[0]) &&
                // (foreach i in 1..args.length-1) {a[i-1] = Integer.parseInt(args[i]) &&
                // (foreach i in 1..k-1) {a[i] < a[i-1]} &&
    ...
    public static int iterativeSearch(int[] a) {
        ...
        // (foreach i in k+1..a.length-1) {a[i] < a[i-1]} && a' == a
        int l = 0;
        // a.length > 0 && (foreach i in 1..k-1) {a[i] < a[i-1]} &&
        // (foreach i in k+1..a.length-1) {a[i] < a[i-1]} && (foreach i in 0..l-1) {a[i] > key} && l >= 0
        // && a' == a
        int r = a.length - 1;
        // a.length > 0 && (foreach i in 1..k-1) {a[i] < a[i-1]} &&
        // (foreach i in k+1..a.length-1) {a[i] < a[i-1]} && (foreach i in 0..l-1) {a[i] > key} &&
